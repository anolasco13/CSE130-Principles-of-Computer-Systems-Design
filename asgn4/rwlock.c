...
void reader_lock(rwlock_t *lock) {
    ...
    while (lock->active_writers > 0 || (lock->mode == WRITERS && lock->waiting_writers > 0)
           || (lock->mode == N_WAY && lock->waiting_writers > 0
               && lock->completed_reads >= lock->threshold)
...
void writer_lock(rwlock_t *lock) {
    ...
    while (lock->active_readers > 0 || lock->active_writers > 0
           ...
           || (lock->mode == N_WAY && lock->waiting_readers > 0
               && lock->completed_reads < lock->threshold)) {
